...
export class ClustersNewAzureContainer extends Component {
    ...
    return (
      <AltContainer
        stores={{
          projects: () => {
            return {
              store: alt.createStore(immutableUtil(Foo), 'Foo'),
...
export default class ClustersNewAzure extends Component {
  static propTypes = {
  ...
  render() {
    ...
    //   'projectId',
    // ]);
    const canCreate = this.canCreate(undefined /* projectId */);
    return (
      <View style={styles.flex}>
        <HeaderPicker
          prefix={'Subscription: '}
          choices={choices}
          selectedIndex={undefined} // {this.state.selectedProjectIndex}
          navigator={this.props.navigator}
          // onChange={index => {
        ...
        >
          <CollectionView
            ...
            list={Immutable.List()} // alt.stores.GoogleCloudStore.getClusters(projectId)}
            onRefresh={this.onRefresh.bind(this)}
            renderRow={this.renderItem.bind(this)}
            renderHeader={() => {
              return (
                <ListHeader
                  title={intl('azure_clusters_list_header')}
            ...
            renderEmpty={() => (
              <EmptyView
                title={intl('azure_clusters_empty_title')}
                subtitle={intl('azure_clusters_empty_subtitle')}
                actionTitle={intl('azure_clusters_empty_action')}
  ...
  canCreate(projectId) {
    console.log(projectId);  // suppress lint error
    return true;
  ...
  submitCluster(cluster) {
    console.log(cluster);  // suppress lint error
  ...
  addCluster(googleCluster) {
    console.log(googleCluster); // suppress lint error
